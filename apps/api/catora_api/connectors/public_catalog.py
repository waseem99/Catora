...
class _ProductHtmlParser(HTMLParser):
    ...
    def handle_data(self, data: str) -> None:
        ...
        if self._ignored_depth == 0 and len(" ".join(self.visible_parts)) < 12_000:
...
class PublicCatalogConnector(CatalogConnector):
        ...
        *,
        ...
        for index, url in enumerate(urls[start_index:], start=start_index):
            try:
                ...
                if record is None:
                    rejections.append(
                        ConnectorRejection(index + 1, "No product evidence found", {"url": url})
                    )
                else:
                    records.append(record)
            except (PublicCatalogConnectorError, httpx.HTTPError) as exc:
                rejections.append(
                    ConnectorRejection(index + 1, str(exc), {"url": url})
                )
            processed += 1
            if self.config.crawl_delay_seconds > 0 and index < len(urls) - 1:
                await self._sleep(self.config.crawl_delay_seconds)
    ...
    async def _discover_urls(self, *, validation_only: bool) -> tuple[str, ...]:
        if self.config.source_type == "urls":
            candidates = self.config.product_urls[: self.config.max_products]
            normalized = [await self._validated_url(url) for url in candidates]
            return tuple(dict.fromkeys(normalized))

        sitemap_url = await self._validated_url(cast(str, self.config.start_url))
        discovered: list[str] = []
        pending = [sitemap_url]
        visited: set[str] = set()
        while pending and len(visited) < self.config.max_sitemaps:
            ...
            child_sitemaps, product_urls = self._parse_sitemap(response.content)
            for child in child_sitemaps:
                child_url = await self._validated_url(urljoin(current, child))
                if child_url not in visited:
                    pending.append(child_url)
            for candidate in product_urls:
                product_url = await self._validated_url(urljoin(current, candidate))
    ...
    async def _validated_url(self, value: str) -> str:
        ...
        if host != self._allowed_host:
            raise PublicCatalogConnectorError("Cross-host catalog URLs are not allowed")
        addresses = await self._resolve_host(host)
        if not addresses:
            raise PublicCatalogConnectorError("Catalog host could not be resolved")
        for address in addresses:
            try:
                ip = ipaddress.ip_address(address)
            except ValueError as exc:
                raise PublicCatalogConnectorError("Catalog host resolution was invalid") from exc
            if not ip.is_global:
                raise PublicCatalogConnectorError("Catalog host must resolve to public IP addresses")
        return normalized
    ...
    def _normalize_url(value: str) -> str:
        ...
        if parsed.scheme.lower() != "https" or not parsed.hostname:
            raise PublicCatalogConnectorError("Catalog URLs must use HTTPS")
        if parsed.username or parsed.password or parsed.port is not None:
            raise PublicCatalogConnectorError("Catalog URLs cannot contain credentials or ports")
        host = parsed.hostname.encode("idna").decode("ascii").lower()
        path = parsed.path or "/"
        return urlunparse(("https", host, path, "", parsed.query, ""))

    async def _request(self, url: str, *, expected: str) -> httpx.Response:
        ...
        for _ in range(_MAX_REDIRECTS + 1):
            ...
            if response.status_code in {301, 302, 303, 307, 308}:
                ...
                if not location:
                    raise PublicCatalogConnectorError("Redirect location was missing")
                current = await self._validated_url(urljoin(current, location))
                continue
            response.raise_for_status()
            content_length = response.headers.get("content-length")
            maximum = _MAX_SITEMAP_BYTES if expected == "xml" else _MAX_HTML_BYTES
            if content_length and int(content_length) > maximum:
                raise PublicCatalogConnectorError("Catalog response exceeded size limit")
            if len(response.content) > maximum:
                raise PublicCatalogConnectorError("Catalog response exceeded size limit")
            content_type = response.headers.get("content-type", "").lower()
            if expected == "xml" and not any(
                value in content_type for value in ("xml", "text/plain", "octet-stream")
            ):
                raise PublicCatalogConnectorError("Sitemap response was not XML")
            if expected == "html" and "html" not in content_type:
                raise PublicCatalogConnectorError("Product response was not HTML")
            return response
    ...
    async def _can_fetch(self, url: str) -> bool:
        ...
        if parser is None:
            ...
            try:
                response = await self._request(robots_url, expected="html")
    ...
    @staticmethod
    def _parse_sitemap(content: bytes) -> tuple[tuple[str, ...], tuple[str, ...]]:
        try:
            root = ElementTree.fromstring(content)
        except ElementTree.ParseError as exc:
            raise PublicCatalogConnectorError("Sitemap XML was invalid") from exc
    ...
    def _record_from_html(
        ...
        payload: dict[str, Any] = {
            ...
            "extraction_method": "json_ld" if products else "html_fallback",
    ...
    def _normalize_canonical(self, source_url: str, canonical: str | None) -> str:
        if not canonical:
            return source_url
        try:
            normalized = self._normalize_url(urljoin(source_url, canonical))
        except PublicCatalogConnectorError:
            return source_url
        return normalized if urlparse(normalized).hostname == self._allowed_host else source_url
    ...
    def _find_products(value: object) -> list[Mapping[str, Any]]:
        ...
        if isinstance(value, list):
            for item in value:
                found.extend(PublicCatalogConnector._find_products(item))
            return found
        ...
        for key in ("@graph", "itemListElement", "mainEntity"):
            if key in mapping:
                found.extend(PublicCatalogConnector._find_products(mapping[key]))
        return found
