...
def _require_feature_enabled() -> None:
    if os.environ.get("CATORA_LOCAL_PROFILE_INTELLIGENCE_ENABLED", "false").lower() != (
        "true"
    ):
            ...
            RestaurantLocationIdentity(
                ...
                address=LocalAddress(
                    address_lines=tuple(
                        value
                        for value in (
                            address_payload.get("street_address"),
                            address_payload.get("address_lines"),
                        )
