...
export const CatalogBridgeJsonValueSchema: z.ZodType<CatalogBridgeJsonValue> = z.lazy(
  () =>
    z.union([
      z.string().max(20_000),
      z.number().finite(),
...
export const CatalogBridgeVariantSchema = z
  .object({
    ...
    options: z.record(SafeFieldKeySchema, CatalogBridgeJsonValueSchema).optional(),
    attributes: z.record(SafeFieldKeySchema, CatalogBridgeJsonValueSchema).optional(),
    ...
    for (const [index, image] of (product.images ?? []).entries()) {
      if (image.variantId && !variantIds.has(image.variantId)) {
        context.addIssue({
          ...
          message: "Image variantId must reference a variant in the same product",
        });
      }
    }
  });
